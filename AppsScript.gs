 ...
 * Tính năng:
 *   1. Văn mẫu NVN      → sheet "Văn mẫu - NVN"
 *   2. Task Pending NVN  → sheet "Công việc Pending"
...
// ─── Helper: Mở sheet, tự tạo nếu chưa tồn tại ───────────────────────────────
function openOrCreate(name, headers) {
...
function doGet(e) {
  ...
  // Hỗ trợ Fallback GET Bypass CORS: Nếu nhận tham số data, chuyển sang xử lý như POST
  if (e.parameter.data) {
    var params;
    try {
  ...
  // ── Văn mẫu NVN: đọc danh sách ──────────────────────────────────────────
  if (action === "read") {
    var sheet = openOrCreate(VAN_MAU_SHEET, ["STT", "Tiếng Anh", "Tiếng Việt", "Note"]);
    var data  = sheet.getDataRange().getValues();
    var rows  = [];
    if (data.length > 1) {
      rows = data.slice(1).map(function (row, idx) {
        return {
          id: idx + 2,
          english:    row[1] || "",
          vietnamese: row[2] || "",
  ...
  if (action === "read_tasks") {
    var sheet = openOrCreate(TASK_SHEET,
      ["ID", "Hạn cuối", "Issue", "Ghi chú", "Người phụ trách", "Trạng thái", "", "", "Thành viên"]);
    var data  = sheet.getDataRange().getValues();
    var rows  = [];
    if (data.length > 1) {
...
// ─── Hàm xử lý chung cho các thao tác ghi dữ liệu (cả POST trực tiếp và GET fallback) ───
